...
GLuint ScoreBar::getProgramId()
{
    return (LoadShaders("shaders/vshader.glsl", "shaders/fshader.glsl"));
}

int ScoreBar::getLevel(){
    return (gameLevel);
}
...
{   
	...
	/* Load a font */
	if (FT_New_Face(ft, "fonts/finland.ttf", 0, &fontface)) {
		fprintf(stderr, "Could not open font %s\n", "fonts/finland.ttf");
		return 0;
	}

	program = create_program("shaders/vshader.glsl", "shaders/fshader.glsl");
	if(program == 0)
		return 0;
...
void ScoreBar::render_text(const char *text, float x, float y, float sx, float sy)
{
	const char *p;
	gslot = fontface->glyph;
	...
	for (p = text; *p; p++) {
		...
		float x2 = x + gslot->bitmap_left * sx;
		float y2 = -y - gslot->bitmap_top * sy;
		float w = gslot->bitmap.width * sx;
		float h = gslot->bitmap.rows * sy;
		...
		x += (gslot->advance.x >> 6) * sx;
		y += (gslot->advance.y >> 6) * sy;
	}
...
void ScoreBar::display()
{
	...
	FT_Set_Pixel_Sizes(fontface, 0, 12);
